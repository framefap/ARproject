...
public class SvrManager : MonoBehaviour
{
    ...
    private bool ViewMatrixType = true;
	...
	void Awake() {
        ...
        if(API_Module_SDKConfiguration.HasKey("Module_Slam", "ViewMatrixType")){
            ViewMatrixType = API_Module_SDKConfiguration.GetBool("Module_Slam", "ViewMatrixType", 1);
            DebugMy.Log("ViewMatrixType:"+ ViewMatrixType,this,true);
        }
...
//        }
    ...
    IEnumerator startSlam() {
        ...
        SvrManager.Instance.SetPause(false);
    ...
    IEnumerator stopSlam() {
        ...
        SvrManager.Instance.SetPause(true);
        yield return new WaitUntil(() => !SvrManager.Instance.IsRunning);
    ...
    public bool IsTrackingValid {
        get {
            if (SvrManager.Instance && SvrManager.Instance.IsRunning) {
                return !SvrManager.Instance.settings.trackPosition
                || ((SvrManager.Instance.plugin.GetTrackingMode() & (int)SvrPlugin.TrackingMode.kTrackingPosition) == 0)
                || (SvrManager.Instance.status.pose & (int)SvrPlugin.TrackingMode.kTrackingPosition) != 0;
            }
            return false;
        }
    }
